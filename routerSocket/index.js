  ...
  router.get('/statistic', async (req, res) => {
    try {
      const result = await database
        ...
        .join('tb_sektor', 'tb_device.id_sektor', 'tb_sektor.id_sektor')
        .where('tb_indikator.nama_indikator', req.query.indikator)
        .orderBy('monitor_dc.id_monitor_dc', 'desc')
        .groupBy('tb_device.id_device')
      ...
      for (let i = 0; i < result.length; i++) {
        const data = await database
          ...
          .orderBy('monitor_dc.id_monitor_dc', 'asc')
          .groupByRaw('DATE_FORMAT(monitor_dc.waktu, "%Y-%m-%d %H")')
          .limit(100)

        result[i].data = data.map((item) => {
          // format unix timestamp
          const date = moment(item.waktu).format('YYYY-MM-DD HH:mm:ss')
          const unix = moment(date).unix() * 1000

          return [unix, item.arus]
        })
